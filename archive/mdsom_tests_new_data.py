...
feature_collections_1 = pd.array([['fixed acidity', 'volatile acidity'], ['citric acid', 'residual sugar'], ['chlorides','free sulfur dioxide']])
...
for _ in range(10):
    trained_soms_layer_1 = train_som_layer(data = X_train, feature_collections = feature_collections_1, n_samples=data_for_evaluation.shape[0])
    ...
    final_som = create_train_som(data= convolv_layer_one_train, n_samples=data_for_evaluation.shape[0], n_features = convolv_layer_one_train.shape[1]*2, convolutional_layer=True)
